{{-- filepath: resources/views/admin/products/variations/edit.blade.php --}}
@extends('admin.layouts.master')

@section('content')
<div class="page-heading">
    <h3>Sửa biến thể cho sản phẩm: {{ $product->ten }}</h3>
</div>
<div class="page-content">
    <form action="{{ route('admin.products.variations.update', [$product->id, $variation->id]) }}" method="POST">
        @csrf
        <div class="card">
            <div class="card-body row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="ma_bien_the" class="form-label">Mã biến thể</label>
                        <input type="text" name="ma_bien_the" class="form-control" value="{{ $variation->ma_bien_the }}">
                    </div>
                    <div class="mb-3">
                        <label for="mau_sac" class="form-label">Màu sắc</label>
                        <input type="text" name="mau_sac" class="form-control" value="{{ $variation->mau_sac }}">
                    </div>
                    <div class="mb-3">
                        <label for="ma_mau" class="form-label">Mã màu</label>
                        <input type="text" name="ma_mau" class="form-control" value="{{ $variation->ma_mau }}">
                    </div>
                    <div class="mb-3">
                        <label for="size_cm" class="form-label">Size (cm)</label>
                        <input type="number" step="0.1" name="size_cm" class="form-control" value="{{ $variation->size_cm }}">
                    </div>
                    <div class="mb-3">
                        <label for="size_eu" class="form-label">Size (EU)</label>
                        <input type="text" name="size_eu" class="form-control" value="{{ $variation->size_eu }}">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="gia_ban" class="form-label">Giá bán</label>
                        <input type="number" name="gia_ban" class="form-control" required value="{{ $variation->gia_ban }}">
                    </div>
                    <div class="mb-3">
                        <label for="gia_nhap" class="form-label">Giá nhập</label>
                        <input type="number" name="gia_nhap" class="form-control" value="{{ $variation->gia_nhap }}">
                    </div>
                    <div class="mb-3">
                        <label for="so_luong" class="form-label">Số lượng</label>
                        <input type="number" name="so_luong" class="form-control" value="{{ $variation->so_luong }}">
                    </div>
                    <div class="mb-3">
                        <label for="trang_thai" class="form-label">Trạng thái</label>
                        <select name="trang_thai" class="form-select">
                            <option value="hien" {{ $variation->trang_thai == 'hien' ? 'selected' : '' }}>Hiện</option>
                            <option value="an" {{ $variation->trang_thai == 'an' ? 'selected' : '' }}>Ẩn</option>
                            <option value="ngung_kinh_doanh" {{ $variation->trang_thai == 'ngung_kinh_doanh' ? 'selected' : '' }}>Ngừng KD</option>
                        </select>
                    </div>
                </div>
            </div>
            <button type="submit" class="btn btn-primary">Cập nhật</button>
            <a href="{{ route('admin.products.variations.index', $product->id) }}" class="btn btn-secondary">Quay lại</a>
        </div>
    </form>
</div>
@endsection